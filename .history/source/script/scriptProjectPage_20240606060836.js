...
function addMilestone(milestoneName) {
    ...
    <img src="../img/2.png" alt="Incomplete Flag" /> 
    `;
...
function updateTimelineProgress() {
    ...
    for (let i = 0; i < timelineLength - 1; i++) {
        if (!timeline[i].classList.contains('completed')) {
    ...
    if (allPreviousCompleted) {
        ...
        if (completedMI == timelineLength - 2) {
            ...
            tlProgress = 100; // Set progress to 100%
            ...
            tlProgress = (completedMI / (timelineLength - 1)) * 100;
        ...
        // If not all previous milestones are completed, reset the progress
        tlProgress = 0;
        let line = document.getElementById('line2');
        line.style.width = tlProgress + '%';
        timeline[timelineLength - 1].classList.remove('completed');
