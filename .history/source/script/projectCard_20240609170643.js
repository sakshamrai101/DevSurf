...
document.addEventListener('DOMContentLoaded', () => {
    ...
    savedProjects.forEach(key => {
        ...
        newCard.setAttribute('project-id', projectData.id); // Set project ID attribute
    ...
    // Append the AddProjectCard at the end
    const addProjectCard = document.createElement('add-project-card');
    projectCards.appendChild(addProjectCard);
...
function saveProjectCards() {
    ...
    projectCards.forEach((card, index) => {
        const projectData = {
            id: index + 1, // Ensure the ID is the index + 1
            name: card.shadowRoot.querySelector('.project-name').value,
        ...
        card.setAttribute('project-id', index + 1); // Set the project ID attribute
    });
...
function loadProjectCards() {
    ...
    projectCardsWrapper.innerHTML = ''; // Clear existing project cards
    projectDataArray.forEach(projectData => {
        const projectCard = new ProjectCard(projectData);
        projectCard.setAttribute('project-id', projectData.id);
        projectCardsWrapper.appendChild(projectCard);
    });
    // Append the AddProjectCard at the end
    const addProjectCard = document.createElement('add-project-card');
    projectCardsWrapper.appendChild(addProjectCard);
}