...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function loadMilestonesAndTasks(){
        ...
        console.log(milestoneList.length);
        for(let i = 0; i < milestoneList.length; i++ ) {
            let taskButton = milestoneList[i].querySelector('.add-task');
            for(let j = 0; j < taskArray[i].length; j++) {
...
window.addEventListener('beforeunload', saveMilestoneToStorage());
window.addEventListener('unload', saveTasksArrayToStorage());