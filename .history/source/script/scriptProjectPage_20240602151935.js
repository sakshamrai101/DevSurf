...
function addMilestone() {
    ...
    if (milestoneList.children.length === 0) {  // Use === instead of =
        milestoneCount = 1;
    } else {
        milestoneCount = milestoneList.children.length + 1;  // Corrected typo
    }

    const newMilestone = document.createElement('li');
    const timelineList = document.getElementById('timeline-elements');
    const newTimelineElement = document.createElement('li');
    ...
    if (timelineCount > 0) {
        ...
        timelineList.appendChild(newTimelineElement);
    }
    ...
    milestoneList.insertBefore(newMilestone, document.querySelector('.add-milestone'));
}
