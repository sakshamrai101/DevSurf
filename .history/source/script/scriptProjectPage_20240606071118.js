...
function updateTimelineProgress() {
    const milestoneList = document.getElementById('milestone-list');
    const compMilestones = milestoneList.querySelectorAll('li.completed');
    ...
    let maxMilestone = 1; 
    console.log(compMilestones[0].getAttribute('data-id'));
    for(let i = 0; i < completedMI; i++) {
        let name = compMilestones[i].getAttribute('data-id');
        console.log(name);
        if(name != `milestone-${maxMilestone}`) {
            break;
        }
        maxMilestone++;
    }
