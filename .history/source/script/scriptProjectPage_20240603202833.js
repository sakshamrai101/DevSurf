...
function updateProgress(milestoneId) {
    ...
    if (completedTasks === tasks.length && tasks.length > 0) {
        ...
        //taskList.closest('ul').appendChild(milestone); //this thing is causing an error
    }
    else 
    {
    ...
    let completedMSNumber = completedMS.length;
    let timelineNumber = timeline.length; 
    let tlProgress = completedMSNumber/timelineNumber;
    let line = getElementById('line');
...
function renumberMilestones() {
    ...
    milestones.forEach((milestone, index) => {
        ...
        const currentName = milestoneNameElement
            .textContent.replace(/\s*\d*$/, ''); // Remove the existing number at the end
