...
function updateProgress(milestoneId) {
    ...
    let timelineElement = document.querySelector(`#timeline-elements [
        data-id="milestone-${milestoneId}"]`);
    ...
    //tracks the line for progress 
    let timelineTotal = document.getElementsById('timeline-elements');
    let completedItems = timelineTotal.querySelectorAll('.completed');
    let uncompletedItems = timelineTotal.querySelectorAll('.uncompleted');
    let completedNumber = completedItems.length - 1 ;
    let uncompletedNumber = uncompletedItems.length;
    const line = document.getElementById('line');
    let progressCompleted = (completedNumber/uncompletedNumber) * 100;
    line.style.background = `linear-gradient(to left, #00FFB0 ${progressCompleted}%, #00FFB0 ${progressCompleted}%, #00FFB0 80%, #00FFB0 80%)`;
}
